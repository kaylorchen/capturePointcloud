...
// Handles all the OpenGL calls needed to display the point cloud
void draw_pointcloud(float width, float height, glfw_state& app_state, rs2::points& points, Eigen::Matrix4f &transform)
{
    if (!points)
        return;
    ...
    for (int i = 0; i < points.size(); i++)
    {
        vec << vertices[i].x, vertices[i].y, vertices[i].z, 1;
        vec = transform*vec;
        v.x = vec(0);
        v.y = vec(1);
        v.z = vec(2);
